...
function init(prefix){inject_hover_for_element(document);var rpl=getReplyTo();rpl.setPrefix(prefix);var e=rpl.elem;var mouseDownX,mouseDownY;var ox=document.body.clientWidth-50;var oy=10;rpl.moveTo(ox,oy);e.setAttribute("draggable","true");e.addEventListener("drag",function(ev){if(ev.shiftKey){rpl.translate(ev.offsetX,ev.offsetY);}else{ev.preventDefault();}},false);e.addEventListener("dragend",function(ev){return;var rpl=getReplyTo();console.log(mouseDownX,mouseDownY,rpl);var ox=rpl.getX();var oy=rpl.getY();var dx=ox+ev.screenX-mouseDownX;var dy=oy+ev.screenY-mouseDownY;console.log(ox,oy,dx,dy);rpl.translate(dx,dy);},false);e=document.getElementById("postform_submit");var postit=function(){var f=document.querySelector("form");var r=getReplyTo();r.showMessage("posting... ");r.post(function(j){if(j.error){r.showError(j.error);}else{r.showMessage("posted :^)");r.updateCaptcha();r.clear();}},function(err){r.showError(err);r.clearSolution();});}
