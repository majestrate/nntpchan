...
table.appendChild(tbody);this.form.appendChild(table);this.elem.appendChild(this.form);document.body.appendChild(this.elem);this.board=null;this.roothash=null;this.prefix=null;this.url=null;this.x=1;this.y=1;}
...
DynReply.prototype.moveTo=function(x,y){console.log("moveto",this,x,y);if(x&&y&&x>0&&y>0){var nx=document.body.clientLeft-x;if(nx>0&&y>0&&nx<document.body.clientWidth&&y<document.body.clientHeight){this.elem.setAttribute("style","top: "+y+"px; right: "+nx+"px;");this.x=x;this.y=y;}}else{console.log("!!",x,y);}}
...
function init(prefix){inject_hover_for_element(document);var rpl=getReplyTo();rpl.setPrefix(prefix);var e=rpl.elem;var mouseDownX,mouseDownY;var ox=document.body.clientWidth-50;var oy=10;rpl.moveTo(ox,oy);e.setAttribute("draggable","true");e.addEventListener("dragstart",function(ev){console.log(ev);mouseDownX=ev.screenX;mouseDownY=ev.screenY;if(!ev.shiftKey){ev.preventDefault();}},false);e.addEventListener("dragend",function(ev){var rpl=getReplyTo();console.log(mouseDownX,mouseDownY,rpl);var ox=rpl.getX();var oy=rpl.getY();var dx=ox+ev.screenX-mouseDownX;var dy=oy+ev.screenY-mouseDownY;console.log(ox,oy,dx,dy);rpl.translate(dx,dy);},false);e=document.getElementById("postform_submit");var postit=function(){var f=document.querySelector("form");var r=getReplyTo();r.showMessage("posting... ");r.post(function(j){if(j.error){r.showError(j.error);}else{r.showMessage("posted :^)");r.updateCaptcha();r.clear();}},function(err){r.showError(err);r.clearSolution();});}
